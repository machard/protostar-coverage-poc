...
func test_proxy_contract{syscall_ptr : felt*, range_check_ptr}():
    ...
    %{ ids.contract_address = deploy_contract("./src/instrumented_storage_contract.cairo", [100, 0, 1]).contract_address %}
    ...
    %{
        print("functions entered :", len(context.COVERAGE["functions"]), "/", context.COVERAGE["total_functions"])
        print("statements entered :", len(context.COVERAGE["statements"]), "/", context.COVERAGE["total_statements"])
    %}
